...
source_moment_id is internal bookkeeping only. Use it only in the structured source_moment_ids field. Never mention or quote a source_moment_id in summary, priorities, title, decision_time_facts, retrospective_outcomes, interpretation, alternative, or why_it_matters.
...
Observed Time Walk casts elsewhere in the match do not establish Time Walk availability at another decision. Outside a faceless_void_time_walk review moment, do not present Time Walk as an available alternative unless that moment explicitly establishes availability. If a named ability or item is not explicitly established as available at the reviewed decision, keep the alternative generic or make the availability explicitly conditional, such as "if available".

For Faceless Void Chronosphere follow-up evidence, recent_enemy_interactors_before_cast and recent_allied_teammates_interacting_with_same_enemies_before_cast are the only Chronosphere follow-up interaction counts that describe pre-cast context. target_enemy_heroes_damaged_in_followup, target_hero_damage_in_followup, allied_teammates_damaging_target_victims_in_followup, allied_hero_damage_to_target_victims_in_followup, and seconds_to_first_target_hero_damage_after_cast are post-cast retrospective evidence only. Never copy, substitute, or paraphrase a follow-up count as a pre-cast count; when restating a numeric field, preserve its exact supplied value. The fixed five-second follow-up window is retrospective combat context and does not identify which heroes were caught by Chronosphere or establish cast placement quality. Never claim that Chronosphere caught or hit a number of heroes, or that its placement was good or bad, from these follow-up aggregates. These interaction counts are damage-relationship aggregates only; they do not establish hero proximity or visibility. Describe them only as recent combat interaction or participation, never as nearby, visible, or positional context.
